import 'package:apptoon/Pages/backup/back-episode.dart';
...
class _DetailCartoonpageState extends State<DetailCartoonpage> {
  ...
  void initState() {
    ...
    // เรียกเมื่อวิดเจ็ตถูกสร้าง
    _fetchEpisodeDetails(widget.episodeData['toonid']);
  }

// ดึงข้อมูลตอนที่ซื้อของผู้ใช้จาก Firestore
  void _fetchEpisodeDetails(String episodeId) async {
    try {
      // ดึงผู้ใช้ปัจจุบันจาก FirebaseAuth
      ...
      if (_user != null) {
        ...
        if (userSnapshot.exists) {
          ...
          // ตรวจสอบว่า episodeId ที่ร้องขอมีใน purchasedEpisodes หรือไม่
          if (purchasedEpisodes.containsKey(episodeId)) {
            ...
            episodes.forEach((key, value) async {
              DocumentSnapshot seriesSnapshot = await FirebaseFirestore.instance
                  .collection('series')
                  .doc(key)
                  .get();
  ...
  //         toonId: widget.episodeData['toonid'],
  //         episodeId: episodeId,
                        ...
                        child: Row(
                          children: [
                            Container(
                              ...
                              child: _purchasedEpisodesData[index]
                                          .containsKey('image') &&
                                      _purchasedEpisodesData[index]['image']
                                          is List &&
                                      (_purchasedEpisodesData[index]['image']
                                              as List)
                                          .isNotEmpty
